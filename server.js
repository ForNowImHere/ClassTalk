...
function addParticipant(id,name,stream=null){
  ...
  if(stream){
    ...
    v.srcObject=stream; div.appendChild(v);
  } else {
    ...
    letter.style.fontSize="48px";letter.style.borderRadius="8px";letter.textContent=name[0].toUpperCase();
    div.appendChild(letter);
  }

  const label=document.createElement("div"); label.className="name-label"; label.textContent=name; div.appendChild(label);

  const micActivity=document.createElement("div"); micActivity.className="mic-activity";
  const micInner=document.createElement("div"); micInner.className="mic-activity-inner"; micActivity.appendChild(micInner); div.appendChild(micActivity);

  grid.appendChild(div);
  participants[id]={div,micInner};
}

function removeParticipant(id){ if(!participants[id]) return; participants[id].div.remove(); delete participants[id]; if(peers[id]){ peers[id].close(); delete peers[id]; } }
...
navigator.mediaDevices.getUserMedia({video:true,audio:true}).then(stream=>{
  localStream=stream; addParticipant("local",username,stream); socket.emit("join",{room,name:username});
}).catch(()=>alert("Camera/Mic required"));

document.getElementById("muteBtn").onclick=()=>{
  muted=!muted; if(localStream) localStream.getAudioTracks()[0].enabled=!muted;
  document.getElementById("muteBtn").textContent=muted?"Mic OFF":"Mic ON";
};
document.getElementById("camBtn").onclick=()=>{
  camOff=!camOff; if(localStream) localStream.getVideoTracks()[0].enabled=!camOff;
  document.getElementById("camBtn").textContent=camOff?"Cam OFF":"Cam ON";
};
document.getElementById("deafenBtn").onclick=()=>{
  deafened=!deafened; Object.values(participants).forEach(p=>p.div.querySelector("video")?.muted=deafened);
...
document.getElementById("screenBtn").onclick=async()=>{
  ...
  try{
    ...
    const screenName=username+"'s screen";
    addParticipant("screen_"+Date.now(),screenName,screenStream);
    Object.values(peers).forEach(pc => screenStream.getTracks().forEach(t=>pc.addTrack(t,screenStream)));
...
socket.on("offer",async d=>{ const pc=await createPeer(d.from); await pc.setRemoteDescription(d.o); const ans=await pc.createAnswer(); await pc.setLocalDescription(ans); socket.emit("answer",{to:d.from,a:ans}); });
